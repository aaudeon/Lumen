...
const isOpen = (levels, progress, levelId, ownedPacks = []) => passageOpen(levels, progress, levelId, DEV_MODE, ownedPacks);

function IslandMap({ selected, progress, levels, onSelect, biome, locked }) {
  const canvasRef = useRef(null);
  const [positions, setPositions] = useState([]);
  useEffect(() => {
    return createMapScene(canvasRef.current, biome.id, mapStops(biome.id, levels.length), levels.map(level=>Boolean(progress[level.id]?.completed)), setPositions, levels);
  ...
  return <div className="expedition-map" aria-label={`Carte : ${biome.name}`}>
    ...
    {levels.map((level,index)=>{
      ...
      return <button key={level.id} data-region={level.region} disabled={shut} className={`map-stop ${done?'completed':''} ${active?'chosen':''} ${shut?'locked':''}`} style={{left:`${at?.x??50}%`,top:`${at?.y??50}%`,visibility:at?'visible':'hidden'}} onClick={()=>onSelect(level.id)} aria-pressed={active} aria-label={`Niveau ${level.chapter} : ${level.name}${shut?`, verrouillé — terminez le niveau ${level.chapter-1}`:done?', terminé':''}`}>
...
export default function HomeScreen({levels,secrets=[],progress,currentGame,busy,error,onStart,onSound,sound,wardrobe,credits=0,onBuy,onEquip,onReset,accountControl,connected=false,packs=[],ownedPacks=[],onBuyPack}) {
  ...
  const startPoint=()=>(!currentGame?.won&&currentGame?.levelId&&isOpen(levels,progress,currentGame.levelId,ownedPacks)
    ? currentGame.levelId : frontierLevel(levels,progress,ownedPacks)?.id);
  const [selected,setSelected]=useState(startPoint);
  const [panel,setPanel]=useState(null);
  const [storeRoom,setStoreRoom]=useState('outfits');
  const [confirmReset,setConfirmReset]=useState(false);
  const closeButton = useRef(null);
  const previousFocus = useRef(null);
  useEffect(()=>{if(!levels.length||busy)return;
    if(!selected||!isOpen(levels,progress,selected,ownedPacks))setSelected(startPoint());},[levels,selected,currentGame,progress,busy,ownedPacks]);
  ...
  const resuming=Boolean(currentGame && currentGame.levelId===selected && currentGame.historyLength>0 && !currentGame.won && !currentGame.lost);
  const retrying=Boolean(currentGame?.lost && currentGame.levelId===selected);
  const done=progress[selected]?.completed;
  const shut=Boolean(levels.length)&&!isOpen(levels,progress,selected,ownedPacks);
  function selectWorld(id) {
    const gate=levels.find(item=>item.biome===id);
    if(gate?.packId&&!ownedPacks.includes(gate.packId)&&!DEV_MODE){setStoreRoom('packs');setPanel('store');return;}
    const candidates=levels.filter(item=>item.biome===id&&isOpen(levels,progress,item.id,ownedPacks));
  ...
  return <main className="expedition-home" data-biome={biome.id} data-region={level?.region}>
    ...
    <header className="home-header">
      ...
      <nav className="biome-tabs" aria-label="Mondes de l’expédition">{BIOMES.map(world=>{
        ...
        const packLocked=gate?.packId&&!ownedPacks.includes(gate.packId)&&!DEV_MODE;
        const pack=packs.find(item=>item.id===gate?.packId);
        const shutWorld=Boolean(levels.length)&&!isOpen(levels,progress,gate?.id,ownedPacks);
        return <button key={world.id} data-world={world.id} className={shutWorld?'locked':''} aria-pressed={biome.id===world.id} disabled={busy||!levels.length||shutWorld&&!packLocked} onClick={()=>selectWorld(world.id)} title={packLocked?'Découvrir le pack dans la boutique':shutWorld?'Ce monde s’ouvre à la fin du monde précédent.':undefined}><span aria-hidden="true">{world.symbol}</span><strong>{world.name}<small>{packLocked?(pack?`${points(pack.price)} pts`:'Pack boutique'):shutWorld?'verrouillé':`${count} / ${size} explorés`}</small></strong>{shutWorld?<i className="lock-mark" aria-hidden="true"/>:count===size?<i aria-hidden="true">✓</i>:null}</button>;
      })}</nav>
    ...
    })}</div><p className="home-modal-note">{DEV_MODE?'Mode dév : tous les passages sont ouverts. Les essais de packs non achetés ne rapportent ni points ni records.':'Les passages s’ouvrent l’un après l’autre. Les Archives nécessitent l’achat de leur pack dans la boutique.'} Les reliques sont facultatives — un niveau se termine sans elles.</p>
    <div className="journal-reset">{confirmReset
      ? <><p>Niveaux terminés, records, points et garde-robe : tout sera effacé, et l’aventure repartira du niveau 01. Les packs de niveaux achetés restent liés au compte. C’est définitif.</p><div className="journal-reset-actions"><button className="journal-reset-go" onClick={onReset}>Oui, tout effacer</button><button onClick={()=>setConfirmReset(false)}>Annuler</button></div></>
