...
const mapPalettes = {
  boreal: {sand:'#d9ebee',cliff:'#4c647e',ground:['#bcd4da','#849fb7','#576f91'],trail:'#eee4c2',water:['#4b859f','#a4d9e3'],waterEdge:'#deedf2',stream:'#adcfe2',sea:'MER DES AURORES',coast:'CRÊTES DE BORÉALE'},
...
function IslandMap({ selected, progress, levels, onSelect, biome, locked }) {
  ...
  useEffect(() => {
    ...
    function paint() {
      ...
      if(biome.id==='boreal') {
        for(let i=0;i<4;i++) {
          ctx.strokeStyle=['#94e3c326','#b4b7ed28','#78bedd24','#a7efdf20'][i];ctx.lineWidth=15+i*4;
          ctx.beginPath();ctx.moveTo(160,90+i*15);ctx.bezierCurveTo(380,6+i*16,600,182+i*8,880,70+i*19);ctx.stroke();
        }
      }
      ...
      function tree(x,y,size){
        ...
        if(biome.id==='boreal') {
          ellipse(x+4,y+5,size*.65,size*.21,'#34496950');
          ctx.fillStyle='#556d87';ctx.beginPath();ctx.moveTo(x-size*.58,y);ctx.lineTo(x,y-size*1.25);ctx.lineTo(x+size*.65,y);ctx.closePath();ctx.fill();
          ctx.fillStyle='#e6f3ed';ctx.beginPath();ctx.moveTo(x-size*.27,y-size*.64);ctx.lineTo(x,y-size*1.25);ctx.lineTo(x+size*.32,y-size*.62);ctx.lineTo(x+size*.06,y-size*.77);ctx.lineTo(x-size*.08,y-size*.58);ctx.closePath();ctx.fill();return;
        }
        if(biome.id==='volcano') {
      ...
      function temple(x,y,size){
        if(biome.id==='boreal') {
          ctx.fillStyle='#4a5168';ctx.fillRect(x-size*.46,y-size*.37,size*.92,size*.4);
          ctx.fillStyle='#e1ede5';ctx.beginPath();ctx.moveTo(x-size*.65,y-size*.36);ctx.lineTo(x,y-size*.85);ctx.lineTo(x+size*.65,y-size*.36);ctx.fill();
          ctx.fillStyle='#edd09a';ctx.fillRect(x-size*.1,y-size*.23,size*.2,size*.25);return;
        }
...
export default function HomeScreen({levels,secrets=[],progress,currentGame,busy,error,onStart,onSound,sound,wardrobe,credits=0,onBuy,onEquip,onReset}) {
